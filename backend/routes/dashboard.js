  ...
  if (userId) {
    try {
      const result = await conn(
        "INSERT INTO product (price, img, name, user_id) VALUES (?, ?, ?, ?)",
        [price, img, name, userId]
      );
      ...
      if (!result) {
        ...
        return res
          .status(400)
          .json({ message: "Hindi matagumpay na idinagdag ang item sa cart" });
      }
  ...
  try {
    if (id) {
      ...
      if (users_unique_item.length > 0) {
        console.log("successful na kinuha ang mga unique na item");
